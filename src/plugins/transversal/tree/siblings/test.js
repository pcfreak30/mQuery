...
describe(".siblings(selector)", function() {
  ...
  beforeEach(function() {
    ...
    expect(mq('.siblings-test').nodes.length).to.equal(2);
    expect(mq('.siblings-test li').nodes.length).to.equal(6);
  });

  afterEach(function() {
    mq('.siblings-test').remove();
    expect(mq('.siblings-test').nodes.length).to.equal(0);
  });
