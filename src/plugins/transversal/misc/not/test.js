describe(".not(elems)", function() {
  ...
  beforeEach(function() {
    ...
    expect(mq('.not-test').length).to.equal(1);
    expect(mq('.not-test li').length).to.equal(3);
  });

  afterEach(function() {
    mq('.not-test').remove();
    expect(mq('.not-test').length).to.equal(0);
  });
  ...
  it("filter single element", function() {
    expect(base.find('.not-test li').not(mq(mq('.not-test li').first())).length).to.equal(2);
  });

  it("filter multiple elements", function() {
    expect(base.find('.not-test li').not(mq('.not-test li.filter')).length).to.equal(1);
  });
